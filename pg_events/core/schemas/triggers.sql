CREATE OR REPLACE FUNCTION pgevents_table_data_update_notify() RETURNS TRIGGER AS $body$
DECLARE
    v_old_data json;
    v_new_data json;
BEGIN
    IF (TG_OP = 'UPDATE') THEN
        ...
        PERFORM pg_notify('pgevents_table_data_update', json_build_object('table_schema', TG_TABLE_SCHEMA::TEXT,'table_name',
...
BEGIN
    ...
    LOOP
        command := format(
            ...
            'CREATE TRIGGER %1$s AFTER INSERT OR UPDATE OR DELETE ON %2$s FOR EACH ROW EXECUTE PROCEDURE pgevents_table_data_update_notify();',
